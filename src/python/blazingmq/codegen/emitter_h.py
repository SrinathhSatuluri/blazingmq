...
from .naming import make_banner, to_class_name, to_enum_value, to_member_name, to_upper_snake
            ...
            for f in t.fields:
                ...
                if f.kind == FieldKind.OPTIONAL:
                    ...
                    if any(
                        ...
                        if isinstance(inner_t, ComplexType) and inner_t.name == ref_name
    ...
    for group in groups:
        ...
        if not overflow:
            ...
            for i, (rf, ob) in enumerate(group):
                if ob in overflow:
                    last_overflow_idx = i
            post_types = [ob for j, (_, ob) in enumerate(group) if j > last_overflow_idx]
...
def _emit_choice_decl(w: Writer, t: ComplexType, schema: Schema, pkg: str) -> None:
    ...
    for i, f in enumerate(t.choices):
        sel_entries.append((f"SELECTION_ID_{to_upper_snake(f.name)}", f.id if f.id is not None else i))
...
def emit_header(  # pylint: disable=redefined-builtin
    ...
    w.line(
        f"// USING bas_codegen.pl -m msg --noAggregateConversion --noExternalization"
    )
...
def _emit_sequence_inline(w: Writer, t: ComplexType, schema: Schema) -> None:
    ...
    if use_hash_helper:
        w.line("// PRIVATE ACCESSORS")
        w.line("template <typename t_HASH_ALGORITHM>")
        _emit_paren_wrap(w, f"void {name}::hashAppendImpl(t_HASH_ALGORITHM& hashAlgorithm) const")
        w.line("{")
        w.line("    using bslh::hashAppend;")
        for rf in resolved:
...
def _emit_choice_inline(w: Writer, t: ComplexType, schema: Schema) -> None:
    ...
    w.line("    default:")
    _emit_bsls_assert(w, f"        BSLS_ASSERT({name}::SELECTION_ID_UNDEFINED == d_selectionId);")
    w.line("        return -1;")
    w.line("    }")
    w.line("}")
    w.line()
